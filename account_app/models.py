...
class OTP(models.Model): 
    ...
    full_name = models.CharField(max_length=200, null=True, blank=True, verbose_name="نام کاربری")
    password = models.CharField(max_length=128, null=True, blank=True, verbose_name="رمز عبور") 
...
class UserManager(BaseUserManager): 
    def create_user(self, phone=None, full_name="", password=None, email=None): 
        if not phone and not email: 
            raise ValueError("داشتن شماره تلفن یا ایمیل الزامی است") 
 
        user = self.model(phone=phone or "00000000000", full_name=full_name, email=email) 
        if password: 
    ...
    def create_superuser(self, phone=None, full_name="", password=None, email=None): 
